...
const BrandsManagement = () => {
  ...
  const endIndex = Math.min(startIndex + itemsPerPage, filteredBrands.length);
  const currentBrands = filteredBrands.slice(startIndex, endIndex);

  const fetchBrands = async () => {
    try {
      setLoading(true);
      const response = await api.get("/marketplace/brands", {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post("/marketplace/brands", formData);
      addModal.closeModal();
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setActionLoading(true);
      await api.put(`/marketplace/brands/${selectedBrand._id}`, formData);
  ...
  const handleDelete = async () => {
    ...
    try {
      setActionLoading(true);
      await api.delete(`/marketplace/brands/${selectedBrand._id}`);
  ...
  return (
    <>
      <PageMeta title="Brands Management | P4P Dashboard" description="Manage marketplace brands" />
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Manage product brands for the marketplace
          </p>
        </div>
        <button
                    ...
                    <TableRow key={brand._id}>
                      <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{brand.name}</span>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] whitespace-nowrap">
                        <Badge variant="light" color={brand.isActive ? "success" : "error"}>
                      ...
                      <TableCell className="px-4 py-3 border border-gray-100 dark:border-white/[0.05] whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <button onClick={() => openEditModal(brand)} className="rounded p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800" title="Edit">
                            <PencilIcon className="h-4 w-4 text-gray-600 dark:text-gray-400" />
                          </button>
                          <button onClick={() => openDeleteModal(brand)} className="rounded p-1.5 hover:bg-error-50 dark:hover:bg-error-900/20" title="Delete">
          ...
          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between">
            <div className="pb-3 xl:pb-0">
              <p className="pb-3 text-sm font-medium text-center text-gray-500 border-b border-gray-100 dark:border-gray-800 dark:text-gray-400 xl:border-b-0 xl:pb-0 xl:text-left">
                Showing {startIndex + 1} to {endIndex} of {filteredBrands.length} entries
              </p>
            </div>
      ...
      {/* Add/Edit Modal */}
      <Modal isOpen={addModal.isOpen || editModal.isOpen} onClose={() => { addModal.closeModal(); editModal.closeModal(); resetForm(); }} className="max-w-[600px]">
        <div className="p-6">
          <h3 className="mb-6 text-2xl font-semibold text-gray-800 dark:text-white">
            {selectedBrand ? "Edit Brand" : "Add New Brand"}
          </h3>
          <form onSubmit={selectedBrand ? handleUpdate : handleCreate} className="space-y-5">
            <div>
            ...
            <div>
              <label htmlFor="logo" className="mb-2.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                Logo URL
              </label>
              <input
                id="logo"
              ...
              <p className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                Enter a URL for the brand logo image
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-4 pt-2">
              <Button variant="outline" onClick={() => { addModal.closeModal(); editModal.closeModal(); resetForm(); }} disabled={actionLoading}>
                Cancel
              </Button>
              <button
              ...
              >
                {actionLoading ? "Saving..." : (selectedBrand ? "Update Brand" : "Create Brand")}
              </button>
            </div>
          </form>
        </div>
      ...
      {/* Delete Modal */}
      <Modal isOpen={deleteModal.isOpen} onClose={deleteModal.closeModal} className="max-w-[500px]">
        <div className="p-6">
          <div className="mb-6">
          ...
          <h3 className="mb-2 text-center text-xl font-semibold text-gray-800 dark:text-white">Delete Brand</h3>
          <p className="mb-6 text-center text-sm text-gray-600 dark:text-gray-400">
            Are you sure you want to delete "<span className="font-semibold">{selectedBrand?.name}</span>"? This action cannot be undone.
          </p>
          <div className="flex items-center justify-center gap-4">
            <Button variant="outline" onClick={deleteModal.closeModal} disabled={actionLoading}>
              Cancel
            </Button>
            <button
